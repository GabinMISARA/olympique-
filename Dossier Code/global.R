...
datajo <- fread("https://raw.githubusercontent.com/GabinMISARA/olympique-/main/Dossier%20Code/athlete_events.csv", 
                select = c("ID", "Sex", "Age", "Team", "NOC", 
                           "Year", "Season", "City", "Sport", "Medal", "Host.country"), 
                sep = ";")

# Données des pays avec leurs coordonnées géographiques
pays <- data.frame(
  nom = c("Norvège", "Suède", "Finlande", "Grande Bretagne", "Pays Bas", 
          "France", "Espagne", "Italie", "Autriche", "Belgique", "Danemark", 
          "Turquie", "Hongrie", "Pologne", "Roumanie", "Canada", "Etats-Unis", 
          "Australie", "Chine", "Japon", "Japon", "Russie", "Brésil", "Argentine"),
  latitude = c(60.472024, 60.128161, 61.92411, 51.509865, 52.132633, 46.603354, 
               40.463667, 41.902782, 47.516231, 50.503887, 56.26392, 38.963745, 
               47.162494, 51.919438, 45.943161, 56.130366, 37.09024, -25.274398, 
               36.204824, 36.204824, 36.204824, 61.52401, -14.235004, -38.416097),
  longitude = c(8.468946, 18.643501, 25.748151, -0.118092, 5.291266, 1.888334, 
                -3.74922, 12.496366, 14.550072, 4.469936, 9.501785, 35.243322, 
                19.503304, 19.145136, 24.96676, -106.346771, -106.346771, 
                133.775136, 104.195397, 138.252924, 138.252924, 138.252924, 
                -51.92528, -63.616672),
  medaille_or = c(192, 205,144, 296, 130, 262, 49, 257, 83, 44, 48, 41, 182, 79,
                  90, 144, 1175, 169, 275, 183, 183, 195, 37, 21 )
)
...
athletes_count_total <- aggregate(cbind(Nb_athletes_Total = seq_along(Year))
                                  ~ Year + Season + Host.country, data = datajo, length)
...
athletes_count_host <- aggregate(cbind(Nb_athletes_Host = seq_along(Year))
                                 ~ Year + Season + Team + Host.country, 
                                 data = datajo_hote, length)


# Partie 5: Fusion des comptes d'athlètes total et par pays hôte
tableau_final_hote <- merge(athletes_count_total, athletes_count_host, 
                            by = c("Year", "Season", "Host.country"), all.x = TRUE)


# Partie 6: Calcul du pourcentage de participation pour chaque pays hôte
tableau_final_hote$Pourcentage_Participation <- (tableau_final_hote$Nb_athletes_Host / 
                                                   tableau_final_hote$Nb_athletes_Total) * 100
...
tableau_final_hote <- merge(tableau_final_hote, medal_count, 
                            by = c("Year", "Season", "Team", "Host.country"), 
                            all.x = TRUE)
...
tableau_final_hote$marker_color <- ifelse(tableau_final_hote$Team == 
                                            tableau_final_hote$Host.country,
                                          "white", "transparent")
tableau_final_hote$marker_size <- ifelse(tableau_final_hote$Team == 
                                           tableau_final_hote$Host.country, 15, 10)

# Renommer les colonnes
names(tableau_final_hote) <- c("Year", "Season", "Team", "Host.country", 
                               "Nb_athletes_Total", "Nb_athletes_Host", 
                               "Pourcentage_Participation", "Medal_Gold", 
                               "Medal_Silver", "Medal_Bronze", "total_Medals", 
...
# Sélectionner les lignes de datajo où la valeur de la colonne "Team" est dans hote_alphab
datajo_Hote <- datajo[datajo$Team %in% hote_alphab, ]
...
datajo3 <- datajo %>%
  ...
  replace_na(list(Medal_Gold = 0, Medal_Silver = 0, Medal_Bronze = 0, total_Medals = 0)) %>%
  filter(Team %in% (filter(., Year >= 2010) %>% group_by(Team) %>% 
                      filter(sum(total_Medals) > 0) %>% pull(Team)))
